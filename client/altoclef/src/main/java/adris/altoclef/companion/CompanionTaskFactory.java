...
final class CompanionTaskFactory {
    ...
    static String validate(CompanionIntent intent) {
        return switch (intent.type()) {
            ...
            case ATTACK -> resolveEntity(intent.target()) == null
                    ? "I do not recognize the entity '" + intent.target() + "'." : null;
            default -> null;
        };
    }
    ...
    static Task create(AltoClef mod, CompanionIntent intent, String owner) {
        return switch (intent.type()) {
            ...
            case ATTACK -> new KillEntitiesTask(entity -> entity.getType() == resolveEntity(intent.target()), Entity.class);
